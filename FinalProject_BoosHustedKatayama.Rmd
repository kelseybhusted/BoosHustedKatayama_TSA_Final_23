...
```{Tidy,  warning=FALSE, message=FALSE}
temp_processed<- temp%>%
...
```{ts,  warning=FALSE, message=FALSE}
#Filter for Brazil 
#Temperature
...
```{ts USA,  warning=FALSE, message=FALSE}
#Filter for USA
#Temperature
...
```{ts AUS,  warning=FALSE, message=FALSE}
...
The positive linear temperature trends for Brazil, the United States, and Australia indicate the presence of climate change effects over time (Seasonal Mann-Kendall: p-values < 0.05). The ARIMA + Fourier and TBATS models were generally the best predictors for temperature changes over time as displayed in the error terms. Future steps might include looking into exogenous variables or events related to temperature such as precipitation, El Nino, and La Nina.

##Limitations

Temperatures were generalized for the entire countries analyzed in this project. This approach may have caused issues since regional climates in countries can contrast significantly. Countries were also selected based on personal biases. Although daily data was available, only monthly temperatures were forecasted. 
